export default {
  ...
  knowledgeEditor: {
    vlmChain: {
      orderLabel: 'VLLM 模型顺序',
      orderDescription: '按顺序尝试图片理解模型，前一个失败、超时或返回空白时自动切换下一个。',
      primaryLabel: '主 VLLM 模型',
      fallbackLabelPrefix: '备用模型',
      primaryPlaceholder: '请选择主 VLLM 模型',
      fallbackPlaceholder: '请选择备用 VLLM 模型',
      addFallback: '添加备用模型',
      up: '上移',
      down: '下移',
      remove: '删除',
      hint: '按顺序尝试，最多 {max} 个；失败、超时或空白响应会自动切换到下一个。',
      duplicateWarning: '该 VLLM 模型已在顺序列表中。'
    },
    titleCreate: '新建知识库',
    titleEdit: '知识库设置',
    advanced: {
      ...
      multimodal: {
        fallbackVllmLabel: '备用 VLLM 模型',
        fallbackVllmDescription: '主 VLLM 调用失败时使用的备用模型，可不填',
        fallbackVllmPlaceholder: '请选择备用 VLLM 模型（可选）',
        label: '多模态功能',
    ...
    wiki: {
      synthesisPrimaryLabel: '主合成模型',
      synthesisFallbackLabelPrefix: '备选模型',
      synthesisFallbackPlaceholder: '选择备用 Wiki 合成模型',
      synthesisAddFallback: '添加备选模型',
      synthesisModelChainHint: '主模型为空时使用摘要模型；模型调用报错时按顺序最多尝试 {max} 个模型。',
      synthesisDuplicateWarning: '该合成模型已在备选链中。',
      title: 'Wiki 设置',
