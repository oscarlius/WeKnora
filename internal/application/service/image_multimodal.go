...
func (s *ImageMultimodalService) Handle(ctx context.Context, task *asynq.Task) error {
	...
	imageVLM, err := s.resolveVLM(ctx, payload.KnowledgeBaseID, payload.KnowledgeID)
	if err != nil {
...
func (s *ImageMultimodalService) resolveVLM(ctx context.Context, kbID, knowledgeID string) (*resolvedImageVLM, error) {
	...
	if vlmCfg.ModelID != "" {
		...
		if len(resolved.candidates) == 0 {
			if len(resolved.resolveErrors) > 0 {
				return nil, fmt.Errorf("no VLM models resolved from chain %v: %s", chainIDs, strings.Join(resolved.resolveErrors, "; "))
			}
			return nil, fmt.Errorf("no VLM models configured for knowledge base %s", kbID)
		}
		return resolved, nil
	}
